...
export default function Approve({
  ...
  const getUserBalance = async () => {
      const odonTokenContract = getOdonTokenContract(library);
      if (account) {
        setTimeout(async () => {
          const res = await odonTokenContract.balanceOf(account);
      ...
      {balance != null ? (
        <div>
          <div className="my-2 break-words">
            Approve spender address: {loanContractAddress}
          </div>
          <div className="my-2 break-words">
          ...
          {balance < information.collateralAmount ? (
            ...
            <div
              ...
              className={"rounded mx-auto mt-10 py-1 md:px-3 md:py-2 bg-3clr-gradient opacity-80 select-none w-32 md:w-40 text-center cursor-pointer text-xl md:text-2xl shadow font-bold hover:opacity-100 transition" + (themeMode ? " text-white":"")}
            >
              Approve
            </div>
          )}
