...
const LendDetail = ({
  ...
  const handleApprove = async () => {
    ...
    const res = await contract
      .approve(
        loanContractAddress,
        ethers.utils.parseUnits(information.amount, decimal)
      )
  ...
  const handleLendToken = async () => {
    ...
    const res = await contract
      .depositToken(ethers.utils.parseUnits(information.amount, decimal), mType)
      .catch((err) => {
        setIsConfirm(false)
        console.log("err", err)
      });
    ...
    >
      ...
      {isDone ? (
        ...
        <div>
          ...
          <div className="my-2">
            {" "}
            {mType === 2 ? (usdcAccBalance >= parseFloat(information.amount) ? (
              isApproving ? (
                <div className="w-[50px] h-[50px] my-5 border-t-2 border-teal-600 animate-spin rounded-full  mx-auto"></div>
              ) : (
                <div
            ...
            {mType === 3 ? (usdtAccBalance >= parseFloat(information.amount) ? (
              isApproving ? (
                <div className="w-[50px] h-[50px] my-5 border-t-2 border-teal-600 animate-spin rounded-full  mx-auto"></div>
              ) : (
                <div
            ...
            {mType === 4 ? (wbtcAccBalance >= parseFloat(information.amount) ? (
              isApproving ? (
                <div className="w-[50px] h-[50px] my-5 border-t-2 border-teal-600 animate-spin rounded-full  mx-auto"></div>
              ) : (
                <div
...
function Manage() {
  ...
  const handleLend = async () => {
    ...
    if (mType === 2) {
      ...
      validateValue = 0.0001;
    }
    if (lendInputRef.current.value >= validateValue) {
      setIsValidLendAmount(true);
