  ...
  return (
    ...
    >
      ...
      {isDone ? (
        ...
        <div className={(themeMode ? " text-black" : "")}>
          <div>Lend amount: {information.amount}&nbsp;
            {mType === 2 && "USDC"}
            {mType === 3 && "USDT"}
            {mType === 4 && "BTC"}
          </div>
        ...
        <div>
          ...
          <div className="my-2">
            ...
            {mType === 4 ? (getAccWBTCBalance() >= information.amount ? (
              isApproving ? (
                <div className="w-[50px] h-[50px] my-5 border-t-2 border-teal-600 animate-spin rounded-full  mx-auto"></div>
              ) : (
                <div
...
export default function Lend() {
  ...
  const [lendAPY, setLendAPY] = useState(usdcLendApy);
  ...
  const handleDropButtonClick = (id) => {
    if (id === 2) {
      setLendType(2);
      setLendAPY(usdcLendApy);
    } else if (id === 3) {
      setLendType(3);
      setLendAPY(usdtLendApy);
    } else if (id === 4) {
      setLendType(4);
      setLendAPY(wbtcLendApy);
    }
  }
  ...
  return (
    <div className="mt-10 md:mt-24">
      ...
      <div className="mx-5 md:mx-auto w-[fit-content]">
        <div className={"font-bold text-2xl mb-3" + (themeMode ? " text-black" : " text-white")}>Lend Order</div>
        <div className={"p-4 shadow-xl  h-auto rounded-xl" + (themeMode ? " bg-white text-black" : " bg-[#191b1fc2] text-white")}>
          <div>
            <p className="my-1 mb-2">How much do you want to lend?</p>
            ...
            <div className={"p-0 flex justify-center items-center" + (themeMode ? " text-[#a78913]":" text-yellow-300")}>
              ...
              Only lend for more than 30 days can get {lendAPY ? lendAPY + "%" : "Fetching..."} interest when
                withdraw !
              </div>
            </div>
            {!account ? (
